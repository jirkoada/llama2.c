...
class Transformer(nn.Module):
    ...
    def __init__(self, params: ModelArgs):
        ...
        self.rnn_down= nn.LSTM(params.vocab_size, 128) #, barch_first=True)
        #self.rnn_up = nn.LSTM(128, params.vocab_size, barch_first=True)
        self.ffup = nn.Linear(128, params.vocab_size, bias=False)
    ...
    def forward(self, tokens: torch.Tensor, targets: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if targets is not None:
            ...
            if self.training:
                h_0 = torch.autograd.Variable(torch.zeros(1, 256, 128))
                c_0 = torch.autograd.Variable(torch.zeros(1, 256, 128))
                rnn_out, (_, _) = self.rnn_down(logits, (h_0, c_0))
                #logits2 = self.rnn_up(rnn_out)[:, :-1, :]
                logits2 = self.ffup(rnn_out)[:, :-1, :]
                ...
                self.last_loss = 0.7*self.first_step_loss + 0.3*self.second_step_loss
            else:
                self.last_loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=-1)
        else:
